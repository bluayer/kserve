...
func TestLocalModelCacheDeploymentReconciler_CreateLocalModelCache(t *testing.T) {
	...
	if cache.Spec.SourceModelUri != "s3://bucket/model" {
		t.Errorf("Expected sourceModelUri s3://bucket/model, got %s", cache.Spec.SourceModelUri)
	}
...
func TestLocalModelCacheDeploymentReconciler_NewRevision(t *testing.T) {
	...
	deployment := &v1alpha1.LocalModelCacheDeployment{
		...
		Spec: v1alpha1.LocalModelCacheDeploymentSpec{
			SourceModelUri: "s3://bucket/model-v2",
	...
	if newCache.Spec.SourceModelUri != "s3://bucket/model-v2" {
		t.Errorf("Expected sourceModelUri s3://bucket/model-v2, got %s", newCache.Spec.SourceModelUri)
	}
